...
void CWndShadow::SetShadowStatus(UINT nState,BOOL bAddOrRemove)
{
	if (bAddOrRemove)
	{
		m_Status |= nState;
	...
	if(SS_VISABLE & m_Status)
		Update(GetParent(m_hWnd));
	Show(GetParent(m_hWnd));
}

void CWndShadow::ShowShadowWindow(BOOL bShowWindow)
{
	if (bShowWindow)
	{
		m_Status |= SS_VISABLE |SS_ENABLED | SS_DISABLEDBYAERO;
		
	}else
	{
		m_Status &= (15-SS_VISABLE);
	}
    Update(GetParent(m_hWnd));
	Show(GetParent(m_hWnd));
}